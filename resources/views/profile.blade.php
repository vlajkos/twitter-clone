<x-app-layout>
    <x-slot name="header">
        ...
        <header class="mx-auto max-w-sm relative">
            ...
            <div class="flex">
                <x-profile-photo :user=$user class="mr-2 mt-1" customClass="w-12 h-12" />
                <div>
                    ...
                    @elseif (request()->user()->following->contains($user))
                        <form action="/unfollow" method="POST" class="absolute right-0 top-0">
                            ...
                            <button type="submit"
                                class="px-5 py-2 bg-black rounded-full font-bold text-white hover:bg-slate-800 mt-1">Unfollow</button>
                        </form>
                    @else
                        <form action="/follow" method="POST" class="absolute right-0 top-0">
                            ...
                            <input type="hidden" value="{{ $user->id }}" name="following_id">
                            <button type="submit"
                                class="px-5 py-2 bg-black rounded-full font-bold text-white hover:bg-slate-800 mt-1">Follow</button>
                        </form>
                    @endif
